...
public final class DomainTransferRequestFlow implements TransactionalFlow {
  ...
  public EppResponse run() throws EppException {
    ...
    if (period.getValue() == 0) {
      ...
      feesAndCredits =
          Optional.of(pricingLogic.getTransferPrice(registry, targetId, now, existingRecurrence));
    } else {
    ...
    updateAutorenewRecurrenceEndTime(
        existingDomain, existingRecurrence, automaticTransferTime, domainHistoryId);
    Domain newDomain =
        existingDomain
            .asBuilder()
