 ...
 * <h3>Beam</h3>
 *
 * The beam job finds the most recent history entry on or before watermark for each resource type
 * loads the embedded resource from it, which is then projected to watermark time to account for
 * things like pending transfer.
 *
 ...
 * Beam transforms. The ghostryde encryption on the full archived deposits makes life a little more
 * difficult for an attacker. But security ultimately depends on the bucket.
 *
 * <h3>Idempotency</h3>
 *
  ...
  public void run() {
    ...
    if (tm().isOfy()) {
      ...
      pendings.values().stream()
          .collect(toImmutableSetMultimap(PendingDeposit::watermark, e -> e))
          .asMap()
          .forEach(
                      ...
                      new LaunchFlexTemplateParameter()
                          ...
                          .setParameters(
                              new ImmutableMap.Builder<String, String>()
                                  ...
                                  // TODO (jianglai): Investigate turning of public IPs (for which
