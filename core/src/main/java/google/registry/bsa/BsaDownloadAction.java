...
import google.registry.bsa.common.DownloadStage;
  ...
  Void runWithinLock() {
    ...
    switch (jobInfo.job().getNextStage()) {
      case DOWNLOAD:
        try (LazyBlockList block = blockListFetcher.fetch(BLOCK);
          ...
          if (!Objects.equals(fetchedChecksums, actualChecksum)) {
            ...
            jobInfo.updateJobStage(DownloadStage.CHECKSUMS_NOT_MATCH);
            return null;
          }
          jobInfo.updateJobStage(DownloadStage.MAKE_DIFF);
        }
        // Fall through
      case MAKE_DIFF:
        ...
        jobInfo.updateJobStage(DownloadStage.APPLY_DIFF);
        // Fall through
      case APPLY_DIFF:
      ...
      case UPLOAD_UNBLOCKABLE_DOMAINS:
        ...
        jobInfo.updateJobStage(DownloadStage.FINISH_UPLOADING);
        // Fall through
      case FINISH_UPLOADING:
        try (Stream<Order> orders = gcsClient.readOrderDiffs(jobInfo.job().getJobId())) {
          bsaBulkApiClient.completeProcessingOrders(orders);
        }
        jobInfo.updateJobStage(DownloadStage.DONE);
        return null;
      case DONE:
      case NOP:
