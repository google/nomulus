...
public class ExportCommitLogDiffActionTest {
  ...
  void beforeEach() {
    ...
    task.batchSize = 5;
    task.taskQueueUtils = new TaskQueueUtils(new Retrier(new FakeSleeper(new FakeClock()), 3));
  }
  ...
  void testRun_noCommitHistory_onlyUpperCheckpointExported() throws Exception {
    ...
    assertNoTasksEnqueued(ReplayCommitLogsToSqlAction.QUEUE_NAME);
  }

  @Test
  void testRun_regularCommitHistory_exportsCorrectCheckpointDiff() throws Exception {
    exportAndVerifyRegularCommitHistory();
    assertNoTasksEnqueued(ReplayCommitLogsToSqlAction.QUEUE_NAME);
  }
  ...
  @Test
  void testRun_enqueuesSqlReplay() throws Exception {
    RegistryConfig.overrideCloudSqlReplayCommitLogs(true);
    exportAndVerifyRegularCommitHistory();
    assertTasksEnqueued(
        ReplayCommitLogsToSqlAction.QUEUE_NAME,
        new TaskMatcher().url(ReplayCommitLogsToSqlAction.PATH));
  }

  private void exportAndVerifyRegularCommitHistory() throws Exception {
    task.lowerCheckpointTime = oneMinuteAgo;
    task.upperCheckpointTime = now;
