...
public class OidcTokenAuthenticationMechanismTest {
  ...
  @Test
  void testBear_tokenExtractor() throws Exception {
    useBearerMechanism();
    // The token does not have "Bearer " prefix.
    when(request.getHeader(PROXY_AUTHORIZATION)).thenReturn(rawToken);
    assertThat(authenticationMechanism.tokenExtractor.extract(request)).isNull();

    // The token is in the correct format.
    when(request.getHeader(PROXY_AUTHORIZATION))
        .thenReturn(String.format("%s%s", BEARER_PREFIX, rawToken));
    assertThat(authenticationMechanism.tokenExtractor.extract(request)).isEqualTo(rawToken);

    // The token is in the correct format, and under the alternative header.
    when(request.getHeader(PROXY_AUTHORIZATION)).thenReturn(null);
    when(request.getHeader(AUTHORIZATION))
        .thenReturn(String.format("%s%s", BEARER_PREFIX, rawToken));
    assertThat(authenticationMechanism.tokenExtractor.extract(request)).isEqualTo(rawToken);
  }

  private void useIapMechanism() {
    TestComponent component = DaggerOidcTokenAuthenticationMechanismTest_TestComponent.create();
    authenticationMechanism = component.iapHeaderAuthenticationMechanism();
  }

  private void useBearerMechanism() {
    TestComponent component = DaggerOidcTokenAuthenticationMechanismTest_TestComponent.create();
    authenticationMechanism = component.bearerHeaderAuthenticationMechanism();
  }
