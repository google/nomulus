...
public class DomainDeletionTimeCacheTest {
  ...
  void testDomainAvailable_deletedInFuture() {
    persistDomainAsDeleted(persistActiveDomain("domain.tld"), toDateTime(plusDays(clock.now(), 1)));
    assertThat(getDeletionTimeFromCache("domain.tld"))
        .hasValue(toDateTime(plusDays(clock.now(), 1)));
  }
  ...
  void testCache_returnsOldData() {
    ...
    persistDomainAsDeleted(domain, toDateTime(plusDays(clock.now(), 1)));
