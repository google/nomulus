...
import static google.registry.testing.DatastoreHelper.*;
...
public class XjcToDomainResourceConverterTest {
  ...
  public void testConvertDomainResource() throws Exception {
    ...
    ImmutableSet<DesignatedContact> expectedContacts = ImmutableSet.of(
    ...
    DomainResource domain = ofy().transact(new Work<DomainResource>() {
    ...
    assertThat(domain.getGracePeriods().size()).isEqualTo(0);
  ...
  public void testConvertDomainResourceAddPeriod() throws Exception {
    ...
    assertThat(domain.getGracePeriods().size()).isEqualTo(1);
    ...
    assertThat(gracePeriod.getExpirationTime())
        .isEqualTo(xjcDomain.getCrDate().plusDays(5));
  }
  ...
  public void testConvertDomainResourceAutoRenewPeriod() throws Exception {
    ...
    assertThat(gracePeriod.getExpirationTime())
        .isEqualTo(xjcDomain.getUpDate().plusDays(45));
  }
  ...
  public void testConvertDomainResourceRedemptionPeriod() throws Exception {
    ...
    assertThat(gracePeriod.getClientId()).isEqualTo(xjcDomain.getClID());
    assertThat(gracePeriod.getExpirationTime())
        .isEqualTo(xjcDomain.getUpDate().plusDays(30));
  }
  ...
  public void testConvertDomainResourceRenewPeriod() throws Exception {
    ...
    assertThat(gracePeriod.getClientId()).isEqualTo(xjcDomain.getClID());
    assertThat(gracePeriod.getExpirationTime())
        .isEqualTo(xjcDomain.getUpDate().plusDays(5));
  }
  ...
  public void testConvertDomainResourcePendingRestorePeriodUnsupported() throws Exception {
    ...
    thrown.expect(IllegalArgumentException.class,
        "Unsupported grace period status: PENDING_RESTORE");
  ...
  public void testConvertDomainResourceWithHostObjs() throws Exception {
    ...
    .containsExactlyElementsIn(ImmutableSet.of(Key.create(host1), Key.create(host2)));
  }
  ...
  public void testConvertDomainResourceHostNotFound() throws Exception {
    ...
    thrown.expect(IllegalStateException.class,
        "HostResource not found with name 'ns2.example.net'");
    ofy().transact(new VoidWork() {
      public void vrun() {
        XjcToDomainResourceConverter.convertDomain(xjcDomain);
      }
  ...
  public void testConvertDomainResourceRegistrantNotFound() throws Exception {
    persistActiveContact("sh8013");final XjcRdeDomain xjcDomain = loadDomainFromRdeXml("domain_fragment.xml");
  ...
  public void testConvertDomainResourceRegistrantMissing() throws Exception {
    ...
    thrown.expect(IllegalArgumentException.class,
        "Registrant is missing for domain 'example1.example'");
    ofy().transact(new VoidWork() {
      public void vrun() {
        XjcToDomainResourceConverter.convertDomain(xjcDomain);
      }
  ...
  public void testConvertDomainResourceSecDnsData() throws Exception {
    ...
    assertThat(domain.getDsData())
    .isEqualTo(ImmutableSet.of(DelegationSignerData.create(4609, 8, 2, base16()
        .decode("5FA1FA1C2F70AA483FE178B765D82B272072B4E4167902C5B7F97D46C8899F44"))));
  }
  ...
  public void testConvertDomainResourceHistoryEntry() throws Exception {
    ...
    persistActiveContact("sh8013");
    final XjcRdeDomain xjcDomain = loadDomainFromRdeXml("domain_fragment.xml");
    // First import in a transaction,
    // then verify in another transaction.
    // Ancestor queries don't work within
    // the same transaction.
    DomainResource domain = ofy().transact(new Work<DomainResource>() {
      public DomainResource run() {
        DomainResource domain = XjcToDomainResourceConverter.convertDomain(xjcDomain);
    ...
    try (InputStream ins = new ByteArrayInputStream(entry.getXmlBytes())) {
      XjcRdeDomain unmarshalledXml =
          ((XjcRdeDomainElement) unmarshaller.unmarshal(ins)).getValue();
  ...
  public void testConvertDomainResourceAutoRenewBillingEvent() throws Exception {
    ...
    assertThat(historyEntries.size()).isEqualTo(1);
    HistoryEntry entry = historyEntries.get(0);
    List<BillingEvent.Recurring> billingEvents = ofy().load().type(BillingEvent.Recurring.class)
        .ancestor(entry).list();
    assertThat(billingEvents.size()).isEqualTo(1);
  ...
  public void testConvertDomainResourceAutoRenewPollMessage() throws Exception {
    ...
    assertThat(historyEntries.size()).isEqualTo(1);
    HistoryEntry entry = historyEntries.get(0);
    List<PollMessage> pollMessages = ofy().load().type(PollMessage.class)
        .ancestor(entry).list();
    assertThat(pollMessages.size()).isEqualTo(1);
    PollMessage pollMessage = pollMessages.get(0);
    assertThat(pollMessage).isInstanceOf(PollMessage.Autorenew.class);
    assertThat(((PollMessage.Autorenew) pollMessage).getTargetId())
        .isEqualTo(xjcDomain.getRoid());
