 ...
 * EppResource#cloneProjectedAtTime(Instant)} it will not actually be re-persisted to the
 * database. Thus, the only actual changes occur when objects are changed by projecting them to now,
 * such as when a pending transfer is resolved.
 */
