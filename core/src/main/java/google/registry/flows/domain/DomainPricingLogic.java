...
public final class DomainPricingLogic {
  ...
  private BaseFee computeCreateFee(
      Tld tld,
      String domainName,
      Instant dateTime,
      int years,
  ...
  private void maybeAddXapFee(
    ...
    if (recentlyDeletedDomain.isPresent()
        ...
        && recentlyDeletedDomain.get().getDeletionTime().isBefore(dateTime)
        && !wasDeletedDuringAddGracePeriod(recentlyDeletedDomain.get(), tld)) {
      Optional<Fee> xapFee =
