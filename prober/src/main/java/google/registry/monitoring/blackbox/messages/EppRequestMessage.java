 ...
 * facets of the 5 basic EPP commands we are attempting to probe. The original 5 are: LOGIN, CREATE,
 * CHECK, DELETE, LOGOUT.</p>
 *
 * <p>In turn, there are 6 similar types: Hello, Login, Create, Check, Delete, and Logout.
 * The only difference is that we added a hello command that simply waits for the server to send a
 * greeting, then moves on to the Login action.</p>
 *
...
public abstract class EppRequestMessage extends EppMessage implements OutboundMessageType {
   ...
   * XML document to a {@link ByteBuf}, the blame falls on the prober, not the server, so it throws
   * an {@link EppClientException}, which is a subclass of the {@link UndeterminedStateException}.
   */
   ...
   * {@link EppResponseMessage.Failure} from server depending on if we expect the command to succeed
   * or fail.</p>
   *
   * <p>Constructor takes in Dagger provided {@code eppClientId} and
   * {@code eppClientPassword} to login to the server. Additionally, the {@link EppResponseMessage}
   * is provided, as that is what we expect to receive back.</p>
   *
