...
- **No Redundant Type Conversions:** Avoid converting between temporal types (like Joda `DateTime` and `Instant`) unnecessarily. Use the correct type as early as possible.
...
- **End of File Newlines:** Ensure all files (especially generated files like XMLs or YAMLs) end with a newline character to avoid GitHub rendering issues.
...
- **Clock Injection:** Never use `System.currentTimeMillis()`. Always use an injected `Clock` instance. When defining timestamps for tests, prefer using a static constant (e.g., `DateTimeUtils.START_INSTANT`) over capturing `clock.now()` to prevent flaky tests.
