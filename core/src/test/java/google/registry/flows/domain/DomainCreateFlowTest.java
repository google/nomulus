...
class DomainCreateFlowTest extends ResourceFlowTestCase<DomainCreateFlow, Domain> {
    ...
    try {
      ...
      setEppInput(
          ...
          new ImmutableMap.Builder<String, String>()
              ...
              .build());
      Instant expectedExpiration = deletionTime.plus(Duration.ofHours(2));
      String expectedDescription =
          String.format("Expiry Access Period, fee expires: %s", expectedExpiration);
      runFlowAssertResponse(
          loadFile(
              "domain_create_response_xap_fee.xml",
              new ImmutableMap.Builder<String, String>()
                  .putAll(FEE_STD_1_0_MAP)
                  .put("FEE_DESCRIPTION", expectedDescription)
                  .build()));
  ...
  void testSuccess_premiumDomainInXap_optedIn_withFeeAck_chargesPremiumAndXapFees()
    ...
    try {
      persistHosts();
      setEppInput("domain_create_premium_xap.xml", FEE_STD_1_0_MAP);
      Instant deletionTime = clock.now().minus(Duration.ofHours(1));
      ...
      runFlowAssertResponse(
          loadFile(
              "domain_create_response_premium_xap.xml",
              new ImmutableMap.Builder<String, String>()
                  .putAll(FEE_STD_1_0_MAP)
                  .put("FEE_DESCRIPTION", expectedDescription)
                  .build()));
