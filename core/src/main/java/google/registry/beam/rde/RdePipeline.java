 ...
 * deposit: pending deposit) pairs.
 *
 * <h3>{@link HostResource}</h3>
 *
 * Similar to {@link ContactResource}, we join the most recent contact history with referenced hosts
 * to find most recent referenced hosts. For external hosts do the same treatment as we did on
 * contacts and obtain the (pending deposit: deposit fragment) pair. For subordinate hosts, we need
 * to include the domain information in the deposit as well. So we first find the superordinate
 * domain repo ID from the host and join the (superordinate domain repo ID: (subordinate host repo
 * ID: (pending deposit: revision ID))) pair with the (domain repo ID: revision ID) pair obtained
 * from the domain history query in order to map the host at watermark to the domain at watermark.
 * We then proceed to create the (pending deposit: deposit fragment) pair for subordinate hosts
 * using the added domain information.
 *
 ...
 * is no need to lock the GCS write to prevent stomping. The cursor for staging the pending deposit
 * is then rolled forward, and the next action is enqueued. The latter two operations are performed
 * in a transaction so the cursor is rolled back if enqueueing failed.
 *
...
public class RdePipeline implements Serializable {
   ...
   *
   * <p>Note that deleted and non-real resources are not included.
   *
                ...
                (KV<String, CoGbkResult> kv) -> {
                  boolean toInclude =
                      ...
                      kv.getValue().getAll(PENDING_DEPOSIT).iterator().hasNext()
                          ...
                          // every referenced resource must be valid at watermark time, otherwise
  ...
  /**
   * Process subordinate hosts by making a deposit fragment with its superordinate domain.
   *
   ...
   *     historical superordinate domain from its history entry and is returned from calling {@link
