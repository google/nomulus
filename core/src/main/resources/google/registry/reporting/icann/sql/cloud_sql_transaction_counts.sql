...
FROM (
    ...
    FROM (
        SELECT
            ...
            -- the cancellation to the owning registrar, but that would require changing the owing
            -- registrar of the the corresponding cancellation DomainHistory, which has cascading
            -- effects that we don't want to deal with. As such we simply floor the number here to
            -- zero to prevent any negative value from appearing, which should have negligible
            -- impact as the edge cage happens very rarely. All the numbers here should be positive.
