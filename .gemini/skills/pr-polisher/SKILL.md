...
5. **Address Errors & Amend**
   If any script throws an error, or if formatting changes were applied, you must stage those fixes and amend your commit:
   ```bash
   git add -u
   git commit --amend --no-edit
   ```
   Loop back to Step 1 until the `check_diff.py` script returns `0 ERRORS`, build and presubmits pass, and the working directory is clean.