...
public final class NordnUploadAction implements Runnable {
  ...
  static final String NORDN_PULL_QUEUE = "nordn-pull-queue";
  ...
  private static final String PARAM_LORDN_PHASE_SUNRISE =
      LordnPhase.SUNRISE.toString().toLowerCase();

  private static final String PARAM_LORDN_PHASE_CLAIMS = LordnPhase.CLAIMS.toString().toLowerCase();
              ...
              () -> {
                List<Domain> domains =
                    tm().createQueryComposer(Domain.class)
                        .where("lordnPhase", EQ, LordnPhase.valueOf(phase.toUpperCase()))
                        .where("tld", EQ, tld)
                ...
                ImmutableList.Builder<Domain> domainBuilder = new ImmutableList.Builder<>();
                ImmutableList.Builder<DomainHistory> historyBuilder = new ImmutableList.Builder<>();
                domains.forEach(
                    domain -> {
                      ...
                      DomainHistory history =
                          new DomainHistory.Builder()
                              ...
                              .setReason("Nordn upload")
                              .setModificationTime(tm().getTransactionTime())
                              .setRegistrarId(domain.getCreationRegistrarId())
                              .setBySuperuser(false)
                              .setRequestedByRegistrar(false)
                              .build();
                      domainBuilder.add(newDomain);
                      historyBuilder.add(history);
                    });
                ImmutableList<Domain> newDomains = domainBuilder.build();
                String columns =
                    phase.equals(PARAM_LORDN_PHASE_SUNRISE) ? COLUMNS_SUNRISE : COLUMNS_CLAIMS;
                String header =
                    String.format("1,%s,%d\n%s\n", clock.nowUtc(), newDomains.size(), columns);
                try {
                  uploadCsvToLordn(String.format("/LORDN/%s/%s", tld, phase), header + csv);
                } catch (IOException | GeneralSecurityException e) {
                  throw new RuntimeException(e);
                }
                tm().updateAll(newDomains);
                tm().insertAll(historyBuilder.build());
              });
    }
  }
