...
public class GaeUserIdConverter extends ImmutableObject {
  ...
  public static String convertEmailAddressToGaeUserId(String emailAddress) {
    ...
    try {
      ...
      // transaction (if any).
      ofyTm().doTransactionless(() -> ofy().saveWithoutBackup().entity(gaeUserIdConverter).now());
      ...
      ofyTm().doTransactionless(() -> ofy().deleteWithoutBackup().entity(gaeUserIdConverter).now());
    }
  }
}
