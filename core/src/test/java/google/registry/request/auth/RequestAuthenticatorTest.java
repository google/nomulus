...
class RequestAuthenticatorTest {
  ...
  private Optional<AuthResult> authResult;
  ...
  private void authorize(AuthLevel authLevel, UserPolicy userPolicy) {
    authResult =
        new RequestAuthenticator(
            ...
            .authorize(
                AuthSettings.create(ImmutableList.of(API, LEGACY), authLevel, userPolicy), req);
  }
  ...
  private void assertAuthResult(@Nullable AuthResult foundResult) {
    if (foundResult == null) {
      assertThat(authResult).isEmpty();
    } else {
      assertThat(authResult).isPresent();
      assertThat(authResult.get()).isEqualTo(foundResult);
    }
  }

  @BeforeEach
  void beforeEach() {
    when(apiAuthenticationMechanism1.authenticate(eq(req))).thenReturn(NOT_AUTHENTICATED);
    when(apiAuthenticationMechanism2.authenticate(eq(req))).thenReturn(NOT_AUTHENTICATED);
    when(legacyAuthenticationMechanism.authenticate(eq(req))).thenReturn(NOT_AUTHENTICATED);
  }
  ...
  void testAuthorize_noneRequired() {
    for (AuthResult resultFound :
      ...
      when(apiAuthenticationMechanism1.authenticate(eq(req))).thenReturn(resultFound);
      authorize(NONE, PUBLIC);
      assertAuthResult(resultFound);
    }
  }
  ...
  void testAuthorize_userAdminRequired() {
    ...
    when(apiAuthenticationMechanism1.authenticate(eq(req))).thenReturn(USER_ADMIN_AUTH);
    authorize(USER, ADMIN);
    assertAuthResult(USER_ADMIN_AUTH);
  }

  @Test
  void testAuthenticate_apiFirst() {
    when(apiAuthenticationMechanism1.authenticate(eq(req))).thenReturn(APP_AUTH);
    assertThat(authenticate(API, LEGACY)).isEqualTo(APP_AUTH);
    verify(apiAuthenticationMechanism1).authenticate(eq(req));
  ...
  void testAuthenticate_apiSecond() {
    when(apiAuthenticationMechanism2.authenticate(eq(req))).thenReturn(APP_AUTH);
    assertThat(authenticate(API, LEGACY)).isEqualTo(APP_AUTH);
    verify(apiAuthenticationMechanism1).authenticate(eq(req));
    verify(apiAuthenticationMechanism2).authenticate(eq(req));
    verifyNoMoreInteractions(apiAuthenticationMechanism1);
    verifyNoMoreInteractions(apiAuthenticationMechanism2);
    verifyNoMoreInteractions(legacyAuthenticationMechanism);
  }
  ...
  void testAuthenticate_legacy() {
    when(legacyAuthenticationMechanism.authenticate(eq(req))).thenReturn(APP_AUTH);
    assertThat(authenticate(API, LEGACY)).isEqualTo(APP_AUTH);
    verify(apiAuthenticationMechanism1).authenticate(eq(req));
    verify(apiAuthenticationMechanism2).authenticate(eq(req));
    verify(legacyAuthenticationMechanism).authenticate(eq(req));
    verifyNoMoreInteractions(apiAuthenticationMechanism1);
    verifyNoMoreInteractions(apiAuthenticationMechanism2);
    verifyNoMoreInteractions(legacyAuthenticationMechanism);
  }
  ...
  void testAuthenticate_returnFirstResult() {
    ...
    // (legacy auth).
    when(apiAuthenticationMechanism2.authenticate(eq(req))).thenReturn(APP_AUTH);
    when(legacyAuthenticationMechanism.authenticate(eq(req))).thenReturn(USER_PUBLIC_AUTH);
    assertThat(authenticate(API, LEGACY)).isEqualTo(APP_AUTH);
    verify(apiAuthenticationMechanism1).authenticate(eq(req));
    verify(apiAuthenticationMechanism2).authenticate(eq(req));
    verifyNoMoreInteractions(apiAuthenticationMechanism1);
  ...
  void testAuthenticate_legacyOnly() {
    when(apiAuthenticationMechanism1.authenticate(eq(req))).thenReturn(USER_PUBLIC_AUTH);
