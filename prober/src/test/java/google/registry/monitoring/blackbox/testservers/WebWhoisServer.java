 ...
 * requisite redirect input, give the client a successful response if they give the expected final
...
public class WebWhoisServer extends TestServer {
  ...
  public WebWhoisServer(LocalAddress localAddress,
      ImmutableList<? extends ChannelHandler> handlers) {
    super(localAddress, handlers);
  }

  public WebWhoisServer(EventLoopGroup eventLoopGroup, LocalAddress localAddress,
      ImmutableList<? extends ChannelHandler> handlers) {
    super(eventLoopGroup, localAddress, handlers);
  }

  /**
   * Creates server that doesn't deal with {@link ByteBuf} conversion and just sends the
   * HttpRequestMessage object through pipeline
   */
  public static WebWhoisServer strippedServer(EventLoopGroup eventLoopGroup,
  ...
  /**
   * Creates server that sends exactly what we expect a remote server to send as a response, by
   * sending the {@link ByteBuf} of the response through pipeline
   */
  public static WebWhoisServer fullServer(EventLoopGroup eventLoopGroup, LocalAddress localAddress,
      String redirectInput, String destinationInput) {
    return new WebWhoisServer(
        eventLoopGroup,
        localAddress,
        ImmutableList.of(
