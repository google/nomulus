...
public class PricingEngineProxyTest {
  ...
  void testGetDomainCreateCost() {
    ...
    assertThat(getDomainCreateCost("rich.example", clock.now(), 1))
        .isEqualTo(Money.of(USD, 100));
    assertThat(getDomainCreateCost("rich.example", clock.now(), 2))
        ...
        Tld.get("example")
            ...
            .setRenewBillingCostTransitions(
                ImmutableSortedMap.of(
                    START_OF_TIME, Money.of(USD, 8), clock.now(), Money.of(USD, 10)))
            .build());
  ...
  void testFailure_cantLoadPricingEngine() {
    ...
    IllegalStateException thrown =
        assertThrows(
            ...
            () -> getDomainCreateCost("bad.example", clock.now(), 1));
    assertThat(thrown)
        .hasMessageThat()
