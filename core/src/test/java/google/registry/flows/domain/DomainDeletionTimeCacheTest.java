...
public class DomainDeletionTimeCacheTest {
  ...
  void testDomainAvailable_deletedInFuture() {
    persistDomainAsDeleted(persistActiveDomain("domain.tld"), clock.now().plusDays(1));
    assertThat(getDeletionTimeFromCache("domain.tld")).hasValue(clock.now().plusDays(1));
  }
  ...
  void testCache_returnsOldData() {
    ...
    persistDomainAsDeleted(domain, clock.now().plusDays(1));
  ...
  void testCache_expires() {
    Domain domain = persistActiveDomain("domain.tld");
    assertThat(getDeletionTimeFromCache("domain.tld")).hasValue(END_OF_TIME);
    Instant elevenMinutesFromNow = clock.now().plusMinutes(11);
