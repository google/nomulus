...
class ConsoleUserDataActionTest {
  ...
  void testSuccess_getContactInfo() throws IOException {
    ...
    assertThat(jsonObject.get("isAdmin")).isEqualTo(false);
    assertThat(jsonObject.get("technicalDocsUrl")).isEqualTo("test");
    assertThat(jsonObject.get("globalRole")).isEqualTo("FTE");
  }
  ...
  private ConsoleUserDataAction createAction(AuthResult authResult) throws IOException {
    return new ConsoleUserDataAction(authResult, response, GSON, "test");
  }
}
