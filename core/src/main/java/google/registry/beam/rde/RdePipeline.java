...
public class RdePipeline implements Serializable {
  ...
  private static final Counter REFERENCED_HOST_COGBK_COUNTER =
      Metrics.counter("RDE", "Referenced Host");
  ...
  private static final Counter ACTIVE_DOMAIN_COUNTER = Metrics.counter("RDE", "ActiveDomainBase");
  ...
  static <E extends EppResource, H extends HistoryEntry>
    ...
    // revisionId) primary key index. Just make sure not to use batch sizes in the ten thousands
  ...
  private static PCollection<KV<String, CoGbkResult>> removeUnreferencedHost(
      ...
      PCollection<KV<String, Long>> hostHistories) {
    PCollection<KV<String, PendingDeposit>> uniqueReferencedHosts =
        referencedHosts
            .setCoder(KvCoder.of(StringUtf8Coder.of(), PendingDepositCoder.of()))
            .apply("Deduplicate referenced hosts for grouping", Distinct.create());
    return KeyedPCollectionTuple.of(PENDING_DEPOSIT, uniqueReferencedHosts)
    ...
    return domainHistories
        .apply(
            ...
            WithKeys.<Integer, KV<String, Long>>of(
                    kv -> Math.abs(kv.getKey().hashCode() % numShards))
        ...
        .apply(
            ...
            ParDo.of(new ProcessBatchDomainHistoriesFn(marshaller, pendingDeposits, watermark))
                .withOutputTags(DOMAIN_FRAGMENTS, TupleTagList.of(REFERENCED_HOSTS)));
  }

  private static class ProcessBatchDomainHistoriesFn
      extends DoFn<
          KV<ShardedKey<Integer>, Iterable<KV<String, Long>>>,
          KV<PendingDeposit, DepositFragment>> {

    private static final long serialVersionUID = 8492019481029481029L;
    private final RdeMarshaller marshaller;
    private final ImmutableSet<PendingDeposit> pendingDeposits;
    private final Instant watermark;

    ProcessBatchDomainHistoriesFn(
        RdeMarshaller marshaller, ImmutableSet<PendingDeposit> pendingDeposits, Instant watermark) {
    ...
    public void processElement(
      ...
      ImmutableList<KV<String, Long>> domainKeys = ImmutableList.copyOf(element.getValue());
      ImmutableMap<String, Domain> loadedDomains =
          loadResourcesByHistoryEntryIds(domainKeys, Domain.class, DomainHistory.class, watermark);

      for (KV<String, Long> kv : domainKeys) {
  ...
  private PCollectionTuple processHostHistories(
    ...
    return removeUnreferencedHost(referencedHosts, hostHistories)
        ...
        .apply(
            ...
            ParDo.of(new ProcessExternalHostsFn(marshaller, watermark))
  ...
  private PCollection<KV<PendingDeposit, DepositFragment>> processSubordinateHosts(
    ...
    return KeyedPCollectionTuple.of(HOST_TO_PENDING_DEPOSIT, superordinateDomains)
        ...
        .apply(
            "Remove unreferenced Domains",
            Filter.by(
                kv -> {
                  boolean toInclude =
                      !Iterables.isEmpty(kv.getValue().getAll(HOST_TO_PENDING_DEPOSIT))
                          && !Iterables.isEmpty(kv.getValue().getAll(REVISION_ID));
                  if (toInclude) {
        ...
        .apply(
            // Batching only combines elements with the same key, so we need to shard
            "Split subordinate hosts across shards for batched retrieval",
            WithKeys.<Integer, KV<String, CoGbkResult>>of(
                    kv -> Math.abs(kv.getKey().hashCode() % numShards))
                .withKeyType(integers()))
        .apply(
        ...
        .apply(
            ...
            ParDo.of(new ProcessSubordinateHostsFn(marshaller, watermark)));
  }

  private static class ProcessExternalHostsFn
      extends DoFn<
          KV<ShardedKey<Integer>, Iterable<KV<String, CoGbkResult>>>,
          KV<PendingDeposit, DepositFragment>> {

    private static final long serialVersionUID = -2481029481029481029L;
    private final RdeMarshaller marshaller;
    private final Instant watermark;

    ProcessExternalHostsFn(RdeMarshaller marshaller, Instant watermark) {
      this.marshaller = marshaller;
      this.watermark = watermark;
    }
              ...
              .map(
                  kv ->
                      KV.of(
                          kv.getKey(),
                          getSingleRevisionId(
                              HostHistory.class, kv.getKey(), kv.getValue().getAll(REVISION_ID))))
              .collect(toImmutableSet());
      ImmutableMap<String, Host> loadedHosts =
          loadResourcesByHistoryEntryIds(hostKeys, Host.class, HostHistory.class, watermark);
          ...
          Streams.stream(kv.getValue().getAll(PENDING_DEPOSIT))
              ...
              .forEach(
                  pendingDeposit -> {
                    ...
                    receiver.get(EXTERNAL_HOST_FRAGMENTS).output(KV.of(pendingDeposit, fragment));
                  });
        }
      }
    }
  ...
  private static class ProcessSubordinateHostsFn
      extends DoFn<
          KV<ShardedKey<Integer>, Iterable<KV<String, CoGbkResult>>>,
          KV<PendingDeposit, DepositFragment>> {

    private static final long serialVersionUID = 5918239018230912830L;
    private final RdeMarshaller marshaller;
    private final Instant watermark;

    ProcessSubordinateHostsFn(RdeMarshaller marshaller, Instant watermark) {
      this.marshaller = marshaller;
      this.watermark = watermark;
    }
    ...
    public void processElement(
              ...
              .map(
                  kv ->
                      KV.of(
                          kv.getKey(),
                          getSingleRevisionId(
                              DomainHistory.class, kv.getKey(), kv.getValue().getAll(REVISION_ID))))
              .collect(toImmutableSet());
      ImmutableSet<KV<String, Long>> hostKeys =
          Streams.stream(element.getValue())
              .flatMap(
      ...
      for (KV<String, CoGbkResult> kv : element.getValue()) {
        ...
        for (KV<String, CoGbkResult> hostToPendingDeposits :
          ...
          DepositFragment fragment = marshaller.marshalSubordinateHost(host, superordinateDomain);
