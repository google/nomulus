...
public interface RegistryQuery<T> extends Serializable {
   ...
   */
  static <T> RegistryQuery<T> createQuery(String sql, boolean nativeQuery) {
    return () -> {
   ...
   */
  static <T> RegistryQuery<T> createQuery(String jpql, Class<T> clazz) {
    return () -> jpaTm().query(jpql, clazz).getResultStream();
  }
