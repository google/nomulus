...
class GetResourceByKeyCommandTest extends CommandTestCase<GetResourceByKeyCommand> {
  ...
  void testFailure_domain_oneDoesNotExist() {
    ...
    assertThat(thrown)
        .hasMessageThat()
        .contains("Could not load resource for key: VKey<DomainBase>(sqlKey:4-TLD,ofyKey:4-TLD)");
  }
  ...
  void testFailure_contact_oneDoesNotExist() {
    ...
    assertThat(thrown)
        .hasMessageThat()
        .contains(
            "Could not load resource for key: VKey<ContactResource>(sqlKey:3-ROID,ofyKey:3-ROID)");
  }
  ...
  void testFailure_host_oneDoesNotExist() {
    ...
    assertThat(thrown)
        .hasMessageThat()
        .contains(
            "Could not load resource for key: VKey<HostResource>(sqlKey:3-ROID,ofyKey:3-ROID)");
  }
  ...
  void testFailure_keyDoesNotExist() {
    ...
    assertThat(thrown)
        .hasMessageThat()
        .contains("Could not load resource for key: VKey<DomainBase>(sqlKey:2-TLD,ofyKey:2-TLD)");
  }
