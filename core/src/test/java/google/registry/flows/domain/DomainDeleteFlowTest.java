...
class DomainDeleteFlowTest extends ResourceFlowTestCase<DomainDeleteFlow, Domain> {
  ...
  private static final Instant A_MONTH_AGO =
      TIME_BEFORE_FLOW.atZone(java.time.ZoneOffset.UTC).minusMonths(1).toInstant();
  private static final Instant A_MONTH_FROM_NOW =
      TIME_BEFORE_FLOW.atZone(java.time.ZoneOffset.UTC).plusMonths(1).toInstant();
                ...
                .setGracePeriods(
                    ImmutableSet.of(
                        GracePeriod.createForRecurrence(
                            ...
                            domain.getRepoId(),
                            A_MONTH_AGO.plus(java.time.Duration.ofDays(45)),
                            "TheRegistrar",
  ...
  private void assertAutorenewClosedAndCancellationCreatedFor(
    ...
    assertBillingEvents(
        ...
        new BillingCancellation.Builder()
            ...
            .setBillingTime(toDateTime(TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(1))))
  ...
  private BillingEvent createBillingEvent(Reason reason, Money cost) {
    return new BillingEvent.Builder()
        ...
        .setCost(cost)
        .setPeriodYears(2)
        .setEventTime(TIME_BEFORE_FLOW.minus(java.time.Duration.ofDays(4)))
        .setBillingTime(toDateTime(TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(1))))
  ...
  void testSuccess_asyncActionsAreEnqueued() throws Exception {
    ...
    cloudTasksHelper.assertTasksEnqueued(
        ...
        new TaskMatcher()
            ...
            .param(PARAM_RESAVE_TIMES, clock.now().plus(java.time.Duration.ofDays(5)).toString())
            .scheduleTime(toDateTime(clock.now().plusMillis(when.getMillis()))));
  }
  ...
  void testDryRun() throws Exception {
    ...
    setUpGracePeriods(
        GracePeriod.create(
            ...
            domain.getRepoId(),
            TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(1)),
            "TheRegistrar",
            null));
  ...
  void testSuccess_updatedEppUpdateTimeAfterPendingRedemption() throws Exception {
    ...
    Instant redemptionEndTime = domain.getLastEppUpdateTime().plus(java.time.Duration.ofDays(3));
    Domain domainAtRedemptionTime = domain.cloneProjectedAtTime(toDateTime(redemptionEndTime));
    assertAboutDomains()
        .that(domainAtRedemptionTime)
        .hasLastEppUpdateRegistrarId("TheRegistrar")
        .and()
        .hasLastEppUpdateTime(toDateTime(redemptionEndTime));
  }
  ...
  private void assertDeletionPollMessageFor(Domain domain, String expectedMessage) {
    ...
    assertThat(
            getPollMessages(
                "TheRegistrar", toDateTime(deletionTime.minus(java.time.Duration.ofMinutes(1)))))
        .isEmpty();
    assertThat(getPollMessages("TheRegistrar", toDateTime(deletionTime))).hasSize(1);
  ...
  void testSuccess_autorenewPollMessageIsNotDeleted() throws Exception {
    ...
    assertThat(
            getPollMessages(
                "TheRegistrar", toDateTime(deletionTime.minus(java.time.Duration.ofMinutes(1)))))
        .hasSize(1);
    assertThat(getPollMessages("TheRegistrar", toDateTime(deletionTime))).hasSize(2);
  }
        ...
        Tld.get("tld")
            ...
            .setRenewBillingCostTransitionsInstant(
                ImmutableSortedMap.of(
                    START_INSTANT,
                    Money.of(USD, 11),
                    TIME_BEFORE_FLOW.minus(java.time.Duration.ofDays(5)),
                    Money.of(USD, 20)))
            .build());
    setUpAutorenewGracePeriod();
    clock.advanceOneMilli();
  ...
  void testSuccess_pendingTransfer() throws Exception {
    ...
    assertThat(getPollMessages("TheRegistrar", toDateTime(clock.now()).plusMonths(1))).isEmpty();
    ...
    assertThat(gainingPollMessage.getEventTime()).isEqualTo(toDateTime(clock.now()));
    assertThat(
            gainingPollMessage
                .getResponseData()
                .stream()
    ...
    earlierHistoryEntry =
        persistResource(
            earlierHistoryEntry
                .asBuilder()
                .setType(DOMAIN_CREATE)
                .setModificationTime(TIME_BEFORE_FLOW.minus(java.time.Duration.ofDays(2)))
    ...
    earlierHistoryEntry =
        persistResource(
            earlierHistoryEntry
                .asBuilder()
                .setType(DOMAIN_CREATE)
                .setModificationTime(TIME_BEFORE_FLOW.minus(java.time.Duration.ofDays(4)))
                .setDomainTransactionRecords(
                    ImmutableSet.of(
                        DomainTransactionRecord.create(
                            "tld",
  ...
  void testIcannTransactionRecord_noGrace_hasRenewRecord() throws Exception {
    ...
    DomainTransactionRecord renewRecord =
        DomainTransactionRecord.create(
            "tld", TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(1)), NET_RENEWS_3_YR, 1);
    // We don't want to cancel non-add or renew records
    DomainTransactionRecord notCancellableRecord =
        DomainTransactionRecord.create(
            "tld", TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(1)), RESTORED_DOMAINS, 5);
    earlierHistoryEntry =
        persistResource(
            earlierHistoryEntry
                .asBuilder()
  ...
  void testIcannTransactionRecord_inGrace_multipleRecords() throws Exception {
    ...
    DomainTransactionRecord existingRecord =
        DomainTransactionRecord.create(
            "tld", TIME_BEFORE_FLOW.plus(java.time.Duration.ofDays(2)), NET_ADDS_10_YR, 1);
    // Create a HistoryEntry with a later modification time
    persistResource(
        new DomainHistory.Builder()
            .setType(DOMAIN_CREATE)
            .setDomain(domain)
            .setModificationTime(TIME_BEFORE_FLOW.minus(java.time.Duration.ofDays(1)))
            .setRegistrarId("TheRegistrar")
  ...
  void testSuccess_superuserExtension_nonZeroDayGrace_nonZeroDayPendingDelete() throws Exception {
    ...
    assertAboutDomains()
        ...
        .hasExactlyStatusValues(StatusValue.INACTIVE, StatusValue.PENDING_DELETE)
        .and()
        .hasDeletionTime(toDateTime(clock.now().plusMillis(standardDays(19).getMillis())));
    assertThat(resource.getGracePeriods())
        .containsExactly(
            GracePeriod.create(
                GracePeriodStatus.REDEMPTION,
                domain.getRepoId(),
                toDateTime(clock.now().plusMillis(standardDays(15).getMillis())),
                "TheRegistrar",
                null,
                resource.getGracePeriods().iterator().next().getGracePeriodId()));
  ...
  void testSuccess_superuserExtension_zeroDayGrace_nonZeroDayPendingDelete() throws Exception {
    ...
    assertAboutDomains()
        ...
        .hasExactlyStatusValues(StatusValue.INACTIVE, StatusValue.PENDING_DELETE)
        .and()
        .hasDeletionTime(toDateTime(clock.now().plusMillis(standardDays(4).getMillis())));
  ...
  void testSuccess_superuserExtension_nonZeroDayGrace_zeroDayPendingDelete() throws Exception {
    ...
    assertAboutDomains()
        ...
        .hasExactlyStatusValues(StatusValue.INACTIVE, StatusValue.PENDING_DELETE)
        .and()
        .hasDeletionTime(toDateTime(clock.now().plusMillis(standardDays(15).getMillis())));
    assertThat(resource.getGracePeriods())
        .containsExactly(
            GracePeriod.create(
                GracePeriodStatus.REDEMPTION,
