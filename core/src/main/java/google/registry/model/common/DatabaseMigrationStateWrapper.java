...
public class DatabaseMigrationStateWrapper extends CrossTldSingleton
  ...
  // states, with one exception: when in SQL_ONLY, we can only move back to SQL_PRIMARY because we
  // must assume that some writes that occurred in SQL_ONLY will not have propagated to Datastore.
  ...
  /** Sets and persists to Datastore the current database migration stage. */
  public static void set(MigrationState newState) {
    ...
    checkArgument(
        ...
        currentState,
        newState);
    DatabaseMigrationStateWrapper stage = new DatabaseMigrationStateWrapper();
    stage.migrationState = newState;
    ofyTm().transact(() -> ofyTm().put(stage));
  }
