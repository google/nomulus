  ...
  @Parameter(
      ...
      description =
          "A comma-separated list of the IDN tables to use for this TLD. Specify an empty list to"
              + " remove any previously-set tables and to use the default.")
    ...
    for (String tld : tlds) {
      ...
      if (idnTables != null) {
        if (idnTables.equals(ImmutableList.of(""))) {
          builder.setIdnTables(ImmutableList.of());
        } else {
          ImmutableList<String> upperCaseIdnTables =
              idnTables.stream().map(String::toUpperCase).collect(toImmutableList());
          ...
          checkArgument(
              ...
              "IDN tables %s contained invalid value(s). Valid values: %s",
              upperCaseIdnTables,
              validIdnStringValues);
          builder.setIdnTables(
              upperCaseIdnTables.stream().map(IdnTableEnum::valueOf).collect(toImmutableList()));
        }
      }
