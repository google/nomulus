...
public class TimeOfYear extends ImmutableObject implements UnsafeSerializable {
  ...
  public static TimeOfYear fromInstant(Instant instant) {
    Instant nextYear = google.registry.util.DateTimeUtils.leapSafeAddYears(instant, 1);
    ZonedDateTime nextYearZdt = ZonedDateTime.ofInstant(nextYear, java.time.ZoneOffset.UTC);
    TimeOfYear instance = new TimeOfYear();
    instance.timeString =
        String.format(
            "%02d %02d %08d",
            nextYearZdt.getMonthValue(),
            nextYearZdt.getDayOfMonth(),
            (nextYearZdt.getHour() * 3600 + nextYearZdt.getMinute() * 60 + nextYearZdt.getSecond())
                    * 1000
                + nextYearZdt.getNano() / 1000000);
    return instance;
  }
  ...
  public Instant getNextInstanceAtOrAfterInstant(Instant start) {
    ...
    return isAtOrAfter(withSameYear, start)
        ? withSameYear
        : google.registry.util.DateTimeUtils.leapSafeAddYears(withSameYear, 1);
  }
  ...
  public Instant getLastInstanceBeforeOrAtInstant(Instant end) {
    ...
    return isBeforeOrAt(withSameYear, end)
        ? withSameYear
        : google.registry.util.DateTimeUtils.leapSafeSubtractYears(withSameYear, 1);
  }
