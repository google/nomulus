...
public class ExportDropListAction implements Runnable {
  ...
  public void run() {
    ...
    logger.atInfo().log("Exporting domain drop list for open TLDs: %s", openTlds);

    if (openTlds.isEmpty()) {
      logger.atInfo().log("No open TLDs found with invoicing enabled.");
      exportToDrive(CSV_HEADER + "\n");
      return;
    }
        ...
        replicaTm()
            .transact(
                ...
                () ->
                    replicaTm()
                        .getEntityManager()
                        .createQuery(SELECT_UPCOMING_DELETIONS_STATEMENT, Object[].class)
                        .setParameter("openTlds", openTlds)
    ...
    exportToDrive(csvBuilder.toString());
  }
    ...
    try {
      String resultMsg =
          driveConnection.createOrUpdateFile(
              DROPLIST_FILENAME, MediaType.CSV_UTF_8, driveFolderId, csvContent.getBytes(UTF_8));
      logger.atInfo().log("Exporting domain drop list succeeded, response was: %s", resultMsg);
    } catch (IOException e) {
      logger.atSevere().withCause(e).log(
          "Error exporting domain drop list to Drive folder %s", driveFolderId);
    }
  }
}
