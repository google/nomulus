...
import google.registry.request.auth.AuthModule.Bearer;
 ...
 *
 * <p>Currently, two flavors are supported. One that checkes for the OIDC token as a bearer token,
 * and another that checks for the OIDC token passed by IAP. In both case, the {@link AuthResult}
 * with the highest {@link AuthLevel} possible is returned. So, if the email address for which the
 * token is minted exists both as a {@link User} and as a service account, the returned {@link
 * AuthResult} is at {@link AuthLevel#USER}.
 *
...
public abstract class OidcTokenAuthenticationMechanism implements AuthenticationMechanism {
   ...
   */
  static class IapHeaderAuthenticationMechanism extends OidcTokenAuthenticationMechanism {

    @Inject
    protected IapHeaderAuthenticationMechanism(
        @Config("serviceAccountEmails") ImmutableList<String> serviceAccountEmails,
        @IAP TokenVerifier tokenVerifier,
        @IAP TokenExtractor tokenExtractor) {
      super(serviceAccountEmails, tokenVerifier, tokenExtractor);
    }
  }

  /**
   * Qualifier for a mechanism to authenticate HTTP requests with an OIDC token as a bearer token.
   *
   ...
   * {@link OAuthAuthenticationMechanism} that is tied to App Engine SDK without having to turn on
   * IAP, which is an all-or-nothing switch for GAE (i.e. no way to turn it on for certain GAE
   * endpoints).
   *
   * <p>Note that this mechanism will try to first extract the token under the
   * "proxy-authentication" header, before trying "authentication". This is because currently the
   * GAE OAuth service always uses "authentication", and we would like to provide a way for both
   * auth mechanisms to be working at the same time for the same request.
   *
   * @see <a href=https://datatracker.ietf.org/doc/html/rfc6750>Bearer Token Usage</a>
   */
  static class BearerHeaderAuthenticationMechanism extends OidcTokenAuthenticationMechanism {

    @Inject
    protected BearerHeaderAuthenticationMechanism(
        @Config("serviceAccountEmails") ImmutableList<String> serviceAccountEmails,
        @Bearer TokenVerifier tokenVerifier,
        @Bearer TokenExtractor tokenExtractor) {
      super(serviceAccountEmails, tokenVerifier, tokenExtractor);
    }
  }
}
