...
class GcsClient {
  ...
  private String saveAndChecksumBlockList(long jobId, LazyBlockList blockList) {
    ...
    try (BufferedWriter gcsWriter = getWriter(blobId)) {
      blockList.read().forEach(line -> writeWithLine(gcsWriter, line));
    } catch (IOException e) {
  ...
  private static void writeWithLine(BufferedWriter writer, String line) {
    try {
      writer.write(line);
      if (!line.endsWith("\n")) {
        writer.write('\n');
  ...
  void writeOrderDiffs(long jobId, Stream<Order> orders) {
    BlobId blobId = getBlobId(jobId, getOrderDiffFileName());
    try (BufferedWriter gcsWriter = getWriter(blobId)) {
      orders.map(Order::serialize).forEach(line -> writeWithLine(gcsWriter, line));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
  ...
  void writeLabelDiffs(long jobId, Stream<Label> labels) {
    BlobId blobId = getBlobId(jobId, getLabelDiffFileName());
    try (BufferedWriter gcsWriter = getWriter(blobId)) {
      labels.map(Label::serialize).forEach(line -> writeWithLine(gcsWriter, line));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
  ...
  void writeUnblockableDomains(long jobId, Stream<UnblockableDomain> unblockables) {
    ...
    try (BufferedWriter gcsWriter = getWriter(blobId)) {
      unblockables
          ...
          .forEach(line -> writeWithLine(gcsWriter, line));
    } catch (IOException e) {
      throw new RuntimeException(e);
    }
  }
