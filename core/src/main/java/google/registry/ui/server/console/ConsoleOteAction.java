...
public class ConsoleOteAction extends ConsoleApiAction {
  ...
  protected void postHandler(User user) {
    ...
    consoleApiParams.response().setPayload(gson.toJson(clientIdToTld));
  }
  ...
  protected void getHandler(User user) {
    ...
    if (!user.getUserRoles().hasGlobalPermission(ConsolePermission.EDIT_REGISTRAR_DETAILS)) {
      setFailedResponse("User doesn't have a permission to create OT&E accounts", SC_BAD_REQUEST);
      return;
    }

    String baseClientId = OteAccountBuilder.getBaseRegistrarId(registrarId);
    tm().transact(
            () -> {
