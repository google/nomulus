...
class DomainCreateFlowTest extends ResourceFlowTestCase<DomainCreateFlow, Domain> {
  ...
  DomainCreateFlowTest() {
    ...
    clock.setTo(Instant.parse("1999-04-03T22:00:00.0Z").minus(Duration.millis(2)));
  }
  ...
  void testSuccess_periodNotSpecified() throws Exception {
    ...
    assertAboutDomains()
        .that(reloadResourceByForeignKey())
        .hasRegistrationExpirationTime(clock.now().plusYears(1));
  ...
  void testFailure_quietPeriodBeforeSunrise_trademarkedDomain() throws Exception {
    ...
    persistResource(
        Tld.get("tld")
            ...
            .setTldStateTransitions(
                ImmutableSortedMap.of(
                    START_OF_TIME, QUIET_PERIOD, clock.now().plusYears(1), START_DATE_SUNRISE))
            .build());
    persistHosts();
    setEppInput("domain_create_allocationtoken_claims.xml");
        ...
        Tld.get("tld")
            ...
            .setTldStateTransitions(
                ImmutableSortedMap.of(
                    START_OF_TIME,
                    QUIET_PERIOD,
                    clock.now().minusYears(1),
                    START_DATE_SUNRISE,
                    clock.now().minusMonths(1),
                    QUIET_PERIOD))
            .build());
    persistHosts();
    setEppInput("domain_create_allocationtoken_claims.xml");
