...
public class IamClient {
  ...
  public void addBinding(String account, String role) {
    ...
    Optional<Binding> binding =
        policy.getBindings().stream().filter(b -> b.getRole().equals(role)).findFirst();
    binding.ifPresentOrElse(
        b -> {
          if (findMember(b, member).isEmpty()) {
        ...
        () ->
            policy
                .getBindings()
                .add(new Binding().setRole(role).setMembers(ImmutableList.of(member))));
    setPolicy(policy);
  }
