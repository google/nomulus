...
public class DnsRefreshRequest extends ImmutableObject {
  ...
  // It would have been nice to use AutoValue, but that would require us to set the ID before the
  // entity is persisted to SQL.
  @Override
  public boolean equals(Object that) {
    if (!(that instanceof DnsRefreshRequest)) {
      return false;
    }
    DnsRefreshRequest other = (DnsRefreshRequest) that;
    if (this == other) {
      return true;
    }
    return id == other.id
        ...
        && tld.equals(other.tld)
        && requestTime.equals(other.requestTime)
        && processTime.equals(other.processTime);
  }
