...
public class UploadBsaUnavailableDomains implements Runnable {
  ...
  void uploadToBsa(String unavailableDomains, DateTime runTime) {
    // POST to https://{Environment_Hostname}/bsa/api/zonefile
    // Body: form-data
    // 1 Key: zone
    // a Value: the checksum of the file that is provided, e.g.
    //
    // {"checkSum":
    // "b5819d248a055912213107f3090a3c1c8a9c2cfdc053b56661cf7a1735cf4c8192a0f855ab5a8
    // 10adc897ffe639d1243e777ac81a796b81c21fe208982d78a50"}
    //
    // b Content-type: application/json
    // 2 Key: file
    // a Value: the compressed unavailable names file

    // CURL example:
    // curl --location 'https://{Environment_Hostname}/bsa/api/zonefile' --header
    // 'Authorization: Bearer TOKEN_GOES_HERE' --form 'zone="{\"checkSum\":
    // "CHECKSUM_GOES_HERE"}";type=application/json' --form
    // 'file=@"FILE_REF_GOES_HERE"'

    // Write encrypted file to bytes.
    // ByteSource(construct).hash(Hashing.sha512())

    try {
      // Need to hash the compressed file.
      String hash =
          ByteSource.wrap(unavailableDomains.getBytes(US_ASCII)).hash(Hashing.sha512()).toString();
      String url = String.format("%s/%s", "TODO base URL", "bsa/api/zonefile");
      ...
      RequestBody body =
          new MultipartBody.Builder()
              ...
              .addFormDataPart(
                  "zone",
                  null,
                  RequestBody.create(String.format("{\"checkSum\": \"%s\"}", hash).getBytes(),
                      MediaType.parse("application/json")))
              .addFormDataPart(
                  "file",
                  "example.csv.gz",
                  RequestBody.create(
                      new File("D://example.csv.gz"), MediaType.parse("application/octet-stream")))
              .build();

      String authBearerToken = "TODO";
      Request request =
          new Request.Builder()
              .url(url)
              .method("POST", body)
      ...
      logger.atInfo().log("Uploading unavailable domains list %s to %s with hash %s", filename, url, hash);
      try (Response response = client.newCall(request).execute()) {
        logger.atInfo().log("Received response with code %s from server: %s", response.code(), response.body().string());
      }
      ...
      connection.setRequestProperty(AUTHORIZATION, "token here");
      // UrlConnectionUtils.setPayloadMultipart(connection, "file", "filename", "application/json");
