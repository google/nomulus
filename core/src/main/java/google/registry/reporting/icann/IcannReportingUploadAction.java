...
public final class IcannReportingUploadAction implements Runnable {
   ...
   *
   * <p>The scheduled time is always the second day of next month at 10 AM UTC. This is because the
  ...
  private ImmutableMap<Cursor, String> defaultNullCursorsToNextMonthAndAddToMap(
    ...
    keyMap.forEach(
        (key, registry) -> {
          // Cursor time is defaulted to 10 AM on the second day of next month since a new tld will
