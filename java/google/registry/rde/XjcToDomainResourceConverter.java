...
final class XjcToDomainResourceConverter extends XjcToEppResourceConverter {
  ...
  private static final Function<String, Key<HostResource>> HOST_OBJ_CONVERTER =
      new Function<String, Key<HostResource>>() {
        ...
        public Key<HostResource> apply(String fullyQualifiedHostName) {
          Key<HostResource> key = ForeignKeyIndex.loadAndGetKey(
              HostResource.class,
  ...
  /** Converts {@link XjcRgpStatus} to {@link GracePeriod} */
  private static class GracePeriodConverter implements Function<XjcRgpStatusType, GracePeriod> {
    ...
    GracePeriodConverter(XjcRdeDomain domain, Key<BillingEvent.Recurring> autoRenewBillingEvent) {
      ...
      this.tld = Registry.get(
          Registries.findTldForNameOrThrow(InternetDomainName.from(domain.getName())).toString());
    }
  ...
  static DomainResource convertDomain(XjcRdeDomain domain) {
    ...
    ofy().save().entities(historyEntry, autoRenewBillingEvent, pollMessage);
    GracePeriodConverter gracePeriodConverter = new GracePeriodConverter(
        domain, Key.create(autoRenewBillingEvent));
    DomainResource.Builder builder = new DomainResource.Builder()
        ...
        .setStatusValues(
            ImmutableSet.copyOf(Iterables.transform(domain.getStatuses(), STATUS_CONVERTER)))
        .setNameservers(convertNameservers(domain.getNs()))
        .setGracePeriods(ImmutableSet
            .copyOf(Iterables.transform(
                domain.getRgpStatuses(), gracePeriodConverter)))
        .setContacts(
            ImmutableSet.copyOf(Iterables.transform(domain.getContacts(), CONTACT_CONVERTER)))
        .setDsData(
            domain.getSecDNS() == null ? ImmutableSet.<DelegationSignerData>of() : ImmutableSet
            .copyOf(Iterables.transform(domain.getSecDNS().getDsDatas(), SECDNS_CONVERTER)))
  ...
  private static BillingEvent.Recurring createAutoRenewBillingEvent(
    ...
    final BillingEvent.Recurring billingEvent = new BillingEvent.Recurring.Builder()
  ...
  private static PollMessage.Autorenew createAutoRenewPollMessage(
      XjcRdeDomain domain, HistoryEntry historyEntry) {
    final PollMessage.Autorenew pollMessage = new PollMessage.Autorenew.Builder()
        .setTargetId(domain.getRoid())
        .setClientId(domain.getClID())
        .setEventTime(domain.getExDate())
  ...
  private static HistoryEntry createHistoryEntry(XjcRdeDomain domain) {
    ...
    final HistoryEntry historyEntry = new HistoryEntry.Builder()
  ...
  private static Key<ContactResource> convertRegistrant(String contactId) {
    Key<ContactResource> key = ForeignKeyIndex.loadAndGetKey(ContactResource.class, contactId, DateTime.now());
    checkState(key != null, "Registrant not found: '%s'", contactId);
    return key;
  }

  /** Converts {@link XjcDomainNsType} to {@link ImmutableSet<Key<HostResource>>}. */
  private static ImmutableSet<Key<HostResource>> convertNameservers(XjcDomainNsType ns) {
    ...
    checkArgument(ns.getHostAttrs() == null || ns.getHostAttrs().isEmpty(),
        "Host attributes are not yet supported");
    return ImmutableSet
          .copyOf(Iterables.transform(ns.getHostObjs(), HOST_OBJ_CONVERTER));
  }
