...
public class TransactionManagerTest {
  ...
  @TestOfyAndSql
  void loadOnlyOf_returnsValue_orEmpty() {
    assertEntityNotExist(theEntity);
    assertThat(transactIfJpaTm(() -> tm().loadOnlyOf(TestEntity.class))).isEmpty();

    tm().transact(() -> tm().insert(theEntity));
    assertThat(transactIfJpaTm(() -> tm().loadOnlyOf(TestEntity.class))).hasValue(theEntity);
  }

  @TestOfyAndSql
  void loadOnlyOf_exceptionOnMultiple() {
    assertAllEntitiesNotExist(moreEntities);
    tm().transact(() -> tm().insertAll(moreEntities));
    assertThat(
            assertThrows(
                IllegalArgumentException.class,
                () -> transactIfJpaTm(() -> tm().loadOnlyOf(TestEntity.class))))
        .hasMessageThat()
        .isEqualTo("Expected at most one entity of type TestEntity, found 3");
  }
