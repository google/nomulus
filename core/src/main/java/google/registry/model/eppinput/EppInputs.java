...
public class EppInputs {
  ...
  public static class DomainCreateBuilder extends ToolEppInputBuilder<DomainCreateBuilder> {
    ...
    public DomainCreateBuilder setNameservers(ImmutableSet<String> nameservers) {
      builder.setNameserverHostNames(ImmutableSortedSet.copyOf(nameservers));
      return this;
    }
  ...
  public static class DomainUpdateBuilder extends ToolEppInputBuilder<DomainUpdateBuilder> {
    ...
    private final ImmutableSortedSet.Builder<String> nameserversToAdd =
        ImmutableSortedSet.naturalOrder();
    private final ImmutableSortedSet.Builder<String> nameserversToRemove =
        ImmutableSortedSet.naturalOrder();
    private final ImmutableSortedSet.Builder<StatusValue> statusesToAdd =
        ImmutableSortedSet.naturalOrder();
    private final ImmutableSortedSet.Builder<StatusValue> statusesToRemove =
        ImmutableSortedSet.naturalOrder();
    private boolean hasInnerChange = false;
    ...
    public boolean hasChanges() {
      boolean hasFunctionalExtension =
          extensions.stream().anyMatch(e -> !(e instanceof MetadataExtension));
      return hasFunctionalExtension
          || hasInnerChange
          || !nameserversToAdd.build().isEmpty()
          || !nameserversToRemove.build().isEmpty()
          || !statusesToAdd.build().isEmpty()
          || !statusesToRemove.build().isEmpty();
    }

    @Override
    public EppInput build() {
      ImmutableSortedSet<String> addNs = nameserversToAdd.build();
      ImmutableSortedSet<StatusValue> addStatus = statusesToAdd.build();
      ...
      ImmutableSortedSet<String> removeNs = nameserversToRemove.build();
      ImmutableSortedSet<StatusValue> removeStatus = statusesToRemove.build();
  ...
  public static class HostCreateBuilder extends ToolEppInputBuilder<HostCreateBuilder> {
    ...
    public HostCreateBuilder setInetAddresses(ImmutableSet<InetAddress> inetAddresses) {
      builder.setInetAddresses(
          ImmutableSortedSet.copyOf(
              Comparator.comparing(InetAddresses::toAddrString), inetAddresses));
      return this;
    }
