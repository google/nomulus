...
public class ConsoleUserDataAction implements JsonGetAction {
  ...
  public void run() {
    ...
    // Both isAdmin and globalRole flags are used by UI as an indicator to hide / show admin
    // specific set of widgets and screens.
    // For example - Admin sees everything, while Registrar sees registrar-specific set of screens
    // This however is merely for visual representation, as any back-end action contains auth checks
    json.put("isAdmin", user.getUserRoles().isAdmin());
    json.put("globalRole", user.getUserRoles().getGlobalRole());

    // Is used by UI to construct a link to registry resources
    json.put("technicalDocsUrl", technicalDocsUrl);
