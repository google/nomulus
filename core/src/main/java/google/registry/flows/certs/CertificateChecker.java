...
public class CertificateChecker {
  ...
  public boolean shouldReceiveExpiringNotification(
     ...
     * + expirationWarningDays AND 2) lastExpiringNotificationSentDate is null (client has not received their
     * first notification) OR lastExpiringNotificationSentDate is not null but the gap between
     * lastExpiringNotificationSentDate and currentDate is greater than expirationWarningIntervalDays.
     *
     */
    return (expirationDate.before(now.plusDays(expirationWarningDays).toDate())
            || expirationDate.equals(now.plusDays(expirationWarningDays).toDate()))
