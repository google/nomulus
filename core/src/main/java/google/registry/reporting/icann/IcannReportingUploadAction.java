...
public final class IcannReportingUploadAction implements Runnable {
  ...
  private void uploadReport(
    ...
    // transaction then later use per-cursor transactions when checking and saving the cursors. We
    ...
    if (!cursor.equals(fromDb)) {
      logger.atSevere().log(
          "Expected previously-loaded cursor %s to equal current cursor %s", cursor, fromDb);
      reportSummaryBuilder.put(filename, false);
      return;
    }
    // Upload the report
