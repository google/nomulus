...
/** Tests for {@link google.registry.ui.server.console.settings.ContactPostAction}. */
public class ContactPostActionTest {
    ...
    ContactPostAction action =
        createPostAction(
            AuthResult.create(
                AuthLevel.USER,
                UserAuthInfo.create(
                    createUser(new UserRoles.Builder()
                        .setRegistrarRoles(
  ...
  private ContactPostAction createPostAction(
      AuthResult authResult, String registrarId, String contacts) {
    when(request.getParameter("contacts")).thenReturn(contacts);
    return new ContactPostAction(
        authResult, RESPONSE, GSON, registrarId, RegistrarConsoleModule.provideContacts(request));
  }
}
