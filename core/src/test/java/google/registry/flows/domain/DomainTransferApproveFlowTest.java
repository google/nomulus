...
class DomainTransferApproveFlowTest
        ...
        Tld.get("tld")
            ...
            .setRenewBillingCostTransitionsInstant(
                new ImmutableSortedMap.Builder<Instant, Money>(Ordering.natural())
                    .put(google.registry.util.DateTimeUtils.START_INSTANT, Money.of(USD, 1))
                    .put(
                        clock.now().minus(java.time.Duration.ofDays(1)).plusMillis(1),
                        Money.of(USD, 22))
  ...
  private void runSuccessfulFlowWithAssertions(
    ...
    assertThat(getPollMessages(domain, "NewRegistrar", toDateTime(clock.now()).plusMonths(1)))
        .hasSize(1);
    assertThat(getPollMessages(domain, "TheRegistrar", toDateTime(clock.now()).plusMonths(1)))
        .hasSize(1);
    ...
    PollMessage gainingTransferPollMessage =
        getOnlyPollMessage(
            domain, "NewRegistrar", toDateTime(clock.now()), PollMessage.OneTime.class);
    ...
    assertThat(gainingTransferPollMessage.getEventTime()).isEqualTo(toDateTime(clock.now()));
  ...
  void testSuccess_removesBulkToken() throws Exception {
    ...
    assertBillingEventsForResource(
        domain,
        new BillingEvent.Builder()
            .setBillingTime(toDateTime(now.plus(java.time.Duration.ofDays(5))))
            .setEventTime(now)
            .setRegistrarId("NewRegistrar")
  ...
  void testSuccess_autorenewBeforeTransfer() throws Exception {
    domain = reloadResourceByForeignKey();
    Instant oldExpirationTime = clock.now().minus(java.time.Duration.ofDays(1));
  ...
  void testFailure_nonexistentDomain() throws Exception {
    deleteTestDomain(domain, toDateTime(clock.now()));
    ResourceDoesNotExistException thrown =
        assertThrows(
            ResourceDoesNotExistException.class,
            () -> doFailingTest("domain_transfer_approve.xml"));
    ...
    assertThat(persistedEntry.getDomainTransactionRecords())
        .containsExactly(
            DomainTransactionRecord.create(
                "tld", clock.now().plus(java.time.Duration.ofDays(3)), TRANSFER_SUCCESSFUL, 1));
  }
  ...
  void testIcannTransactionRecord_cancelsPreviousRecords() throws Exception {
    ...
    DomainTransactionRecord notCancellableRecord =
        DomainTransactionRecord.create(
            "tld", clock.now().plus(java.time.Duration.ofDays(1)), NET_ADDS_4_YR, 5);
    persistResource(
