...
- **Transaction Management:**
    - Define database transactions (`tm().transact(...)`) at the highest possible level in the call chain (e.g., in an Action or a Command).
    - Avoid declaring transactions inside low-level DAO methods or inner utility functions.
    - Inner methods that require a transaction should use `tm().assertInTransaction()` to verify the context without initiating a new transaction.
