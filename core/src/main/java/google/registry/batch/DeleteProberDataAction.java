    ...
    jpaTm()
        .transact(
            () -> {
              ...
              // Scroll through domains, soft-deleting as necessary (very few will be soft-deleted)
              // and keeping track of which domains to hard-delete (there can be many, so we batch
              ...
              for (int i = 0; scrollableResult.next(); i++) {
                ...
                processDomain(
                    domain, domainRepoIdsToHardDelete, hostNamesToHardDelete, softDeletedDomains);
                // Intermittently flush and clear the session so we don't OOM
                if (i % BATCH_SIZE == 0) {
                  jpaTm().getEntityManager().flush();
                  jpaTm().getEntityManager().clear();
                }
              }
              ImmutableList<String> domainRepoIds = domainRepoIdsToHardDelete.build();
              hardDeleteDomainsAndHosts(domainRepoIds, hostNamesToHardDelete.build());
              ...
              logger.atInfo().log(
                  "%s %d domains.",
                  isDryRun ? "Would have hard-deleted" : "Hard-deleted", domainRepoIds.size());
            });
  }
