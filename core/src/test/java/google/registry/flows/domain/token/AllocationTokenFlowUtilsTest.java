...
class AllocationTokenFlowUtilsTest {
  ...
  void testFailure_tokenInvalidForRegistrar() {
    persistResource(
        createOneMonthPromoTokenBuilder(clock.now().minus(Duration.ofDays(1)))
  ...
  void testFailure_beforePromoStart() {
    persistResource(createOneMonthPromoTokenBuilder(clock.now().plus(Duration.ofDays(1))).build());
