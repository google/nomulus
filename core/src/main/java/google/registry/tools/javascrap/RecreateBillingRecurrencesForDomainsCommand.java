...
public class RecreateBillingRecurrencesForDomainsCommand extends ConfirmingCommand {
  ...
  protected String prompt() throws Exception {
    ...
    ImmutableList<BillingRecurrence> recurrences = tm().transact(this::loadRecurrences);
    return String.format(
        "Create new BillingRecurrence(s) for the following BillingRecurrence(s)?\n%s",
        Joiner.on('\n').join(recurrences));
  }
  ...
  private ImmutableList<BillingRecurrence> internalExecute() {
    return loadRecurrences().stream().map(this::convertRecurrence).collect(toImmutableList());
  }

  private BillingRecurrence convertRecurrence(BillingRecurrence existingRecurrence) {
    TimeOfYear timeOfYear = existingRecurrence.getRecurrenceTimeOfYear();
    DateTime newLastExpansion = timeOfYear.getLastInstanceBeforeOrAt(tm().getTransactionTime());
    ...
    BillingRecurrence newRecurrence =
        existingRecurrence
            .asBuilder()
    ...
    // Link the domain to the new BillingRecurrence object
    Domain domain = tm().loadByKey(VKey.create(Domain.class, newRecurrence.getDomainRepoId()));
    tm().put(domain.asBuilder().setAutorenewBillingEvent(newRecurrence.createVKey()).build());
