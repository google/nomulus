...
final class CreateAnchorTenantCommand extends MutatingEppToolCommand {
  ...
  protected void initMutatingEppToolCommand() {
    ...
    addEppInput(
        clientId,
        EppInputs.createDomain(domainName, password)
            .setPeriod(DEFAULT_ANCHOR_TENANT_PERIOD_YEARS, Period.Unit.YEARS)
            .setRegistrant(contact)
            .addAdminTechContact(contact)
            .addExtension(EppExtensions.metadata(reason, false, true))
            .addExtension(EppExtensions.feeCreateV06(cost))
            .build());
  }
}
