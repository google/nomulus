...
public class ExpandBillingRecurrencesPipelineTest {
  ...
  void testSuccess_expandMultipleEvents_multipleDomains(int numOfThreads) {
    ...
    persistResource(
        Tld.get("test")
            ...
            .build());
    Instant otherCreateTime = plusYears(minusYears(startTime, 1), 0).plus(5, ChronoUnit.HOURS);
    // Wait, minusYears(startTime, 1).plus(5, HOURS) is enough.
    otherCreateTime = minusYears(startTime, 1).plus(5, ChronoUnit.HOURS);
  ...
  private static void assertCursorAt(DateTime expectedCursorTime) {
    assertCursorAt(google.registry.util.DateTimeUtils.toInstant(expectedCursorTime));
  }
