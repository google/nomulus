...
final class PasswordUtilsTest {
  ...
  void testVerify_scrypt_default() {
    ...
    assertThat(verifyPassword(password, hashedPassword, base64().encode(salt))).isEqualTo(SCRYPT);
  }
  ...
  void testVerify_sha256() {
    ...
    assertThat(verifyPassword(password, hashedPassword, base64().encode(salt))).isEqualTo(SHA256);
  }
  ...
  void testVerify_failure() {
    ...
    String password = "mySuperSecurePassword";
    String hashedPassword = hashPassword(password, salt);
    assertThat(verifyPassword(password + "a", hashedPassword, base64().encode(salt))).isNull();
  }
}
