...
public class ReadDnsRefreshRequestsActionTest {
  ...
  void testSuccess_runAction_twoBatches() {
    // Make the read batch size 2 = 2 * 1;
  ...
  void testSuccess_runAction_timeOutAfterFirstRead() {
    // Make the process batch size 2 = 2 * 1;
    persistResource(Registry.get("tld").asBuilder().setNumDnsPublishLocks(1).build());
  ...
  void testSuccess_processTasks() {
    ...
    verify(action).enqueueUpdates(2, 2, ImmutableSet.of(requests.get(0), requests.get(1)));
    verify(action).enqueueUpdates(1, 2, ImmutableSet.of(requests.get(2)));
    ...
    cloudTasksHelper
        ...
        .forEach(
            task -> {
              ...
              System.out.println(clock.nowUtc());
