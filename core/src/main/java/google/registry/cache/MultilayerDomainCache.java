...
public class MultilayerDomainCache extends MultilayerEppResourceCache<Domain>
  ...
  protected Domain loadFromDatabase(String domainName) {
    ...
    Domain possiblyDeletedDomain =
        ForeignKeyUtils.loadMostRecentResourceObjects(
                Domain.class, ImmutableList.of(domainName), true)
            .get(domainName);
    if (possiblyDeletedDomain == null
        || DateTimeUtils.isAtOrAfter(now, possiblyDeletedDomain.getDeletionTime())) {
      return null;
    }
    return possiblyDeletedDomain.cloneProjectedAtInstant(now);
  }
